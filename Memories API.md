...
- Memories 서비스의 인증 및 인가와 관련된 API 모듈
- 로그인, 회원가입, 아이디 중복 확인 등의 API가 포함
- Auth 모듈은 인증 없이 요청할 수 있는 모듈  
...
- 클라이언트는 사용자 아이디와 평문의 비밀번호를 포함하여 요청하고, 아이디와 비밀번호가 일치한다면 인증에 사용된 token과 해당 token의 기간 만료 기간을 응답 데이터로 전달받음.
- 만약 아이디와 비밀번호가 아나라도 일치하지 않을 경우 로그인 불일치에 해당하는 응답을 받음.
- 서버 에러, 데이터베이스 에러, 유효성 검사 실패 에러가 발생할 수도 있음 
...
| expiration | Integer | accessToken의 만료 기간 (초 단위) | O |
...
- 클라이언트는 사용할 아이디를 포함하여 요청하고 중복되지 않는 아이디라면 성공 응답을 받음
- 만약 사용중인 아이디라면 아이디 중복에 해당하는 응답을 받음
- 서버 에러, 데이터베이스 에러가 발생할 수 있음
...
- 클라이언트는 사용자 이름, 사용자 아이디, 주소, 상세 주소, 가입 경로를 포함하여 요청하고 회원 가입이 성공적으로 이루어지면 성공에 해당하는 응답을 받음.
- 만약 존재하는 아이디일 경우 중복된 아이디에 대한 응답을 받음
- 서버 에러, 데이터베이스 에러가 발생할 수 있음.
...
| userPassword | String | 사용자 비밀번호 (영문 숫자 조합으로 이루어진 6자 이상 13자 이하 문자열) | O |
| name | String | 사용자 이름 (한글로만 이루어진 2자 이상 5자 이하 문자열) | O |
| address | String | 사용자 주소 | O |
| detailAddress | String | 사용자 상세 주소 | X |
| joinType | String | 가입 경로(NORMAL: 일반, KAKAO: 카카오, NAVERㅣ 네이버) | O |
